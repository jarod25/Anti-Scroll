...
- derives its first lookback window from the configured maximum session duration, inactivity timeout and technical overlap;
...
The polling interval and overlap are Android integration parameters, not restriction business values. They are provided by application composition and can be adjusted after reference-device battery and latency measurements without changing the restriction engine.
...
- background correction can arrive up to the polling and UsageStats publication delay later than the actual transition;
